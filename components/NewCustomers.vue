<template>
	<section class="mt-5">
		...
		<div class="mt-5 flex w-full items-start gap-6 overflow-x-auto rounded-lg border bg-background p-5 scrollbar-thin scrollbar-thumb-input scrollbar-thumb-rounded-md">
			<div class="flex shrink-0 flex-col items-center gap2">
			...
			<template v-for="(d, i) in data.results" :key="i"
				><div class="flex shrink-0 flex-col items-center gap-2">
					<img :src="d.picture.thumbnail" :alt="d.name.first" class="h-10 w-10 rounded-full object-cover" />
					<div class="text-center">
						<p class="text-sm font-semibold">{{ d.name.first }} {{ d.name.last }}</p>
						<p class="text-xs text-muted-foreground">10mins ago</p>
					</div>
				</div>
			</template>
		</div>
